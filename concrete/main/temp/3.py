...
def save_excel_result_with_style(result_list, save_path):
    ...
    # 3. 预先获取原始文件中数字列的格式作为参考
    # 查找一个已有的数字单元格（比如P列第2行），获取其数字格式
    ref_num_cell = ws.cell(row=2, column=16)  # P列第2行（受压钢筋as）
    ref_number_format = ref_num_cell.number_format if ref_num_cell.number_format else "0.00"
    ...
    for idx, result_item in enumerate(result_list):
        ...
        # 获取当前行A列单元格的样式作为参考
        style_ref_cell = ws.cell(row=row_num, column=1)  # A列单元格
        if style_ref_cell:
            ref_font = copy(style_ref_cell.font)
            ref_alignment = copy(style_ref_cell.alignment)
        ...
        for col_key, col_num in col_mapping.items():
            ...
            # 设置数字格式
            cell.number_format = ref_number_format
