...
def save_excel_result_with_style(result_list, save_path):
    ...
    # 3. 定义辅助函数，安全地写入值
    def safe_write_value(worksheet, row, col, value):
        """安全地将值写入Excel单元格"""
        ...
        try:
            ...
            # 如果转换失败，记录错误并写入空值
            print(f"警告：写入单元格({row},{col})时出错: {e}，值: {value}")
            cell.value = ""
    ...
    for idx, result_item in enumerate(result_list):
        ...
        # 安全地写入每个值
        safe_write_value(ws, row_num, col_mapping["x_col"], result_item.get(output_cols["x_col"]))
        safe_write_value(ws, row_num, col_mapping["mu_col"], result_item.get(output_cols["mu_col"]))
        safe_write_value(ws, row_num, col_mapping["mue_col"], result_item.get(output_cols["mue_col"]))
        safe_write_value(ws, row_num, col_mapping["rs_col"], result_item.get(output_cols["rs_col"]))
    ...
    try:
        ...
        try:
            ...
            print(f"✅ 使用pandas备选方案保存Excel结果文件至：{save_path}")
