...
def save_excel_result_with_style(result_list, save_path):
    """保存Excel结果，保留原文件样式，只更新Q-T列数据"""
    # 1. 加载原始Excel文件，保留所有样式
    wb = load_workbook(excel_input_path)
    ws = wb.active

    # 2. 确定Q-T列的列索引
    # 根据您的表格，Q列是第17列（A=1, B=2, ... Q=17）
    col_mapping = {
        "x_col": 17,  # Q列
        "mu_col": 18,  # R列
        "mue_col": 19,  # S列
    ...
    # 3. 获取第一行（标题行）的样式作为参考
    # 使用A列的标题单元格作为样式参考
    style_ref_cell = ws.cell(row=1, column=1)  # A1单元格
    title_font = copy(style_ref_cell.font)
    title_alignment = copy(style_ref_cell.alignment)
    title_fill = copy(style_ref_cell.fill) if style_ref_cell.fill else None

    # 4. 获取数据行的样式参考（使用第二行A列单元格）
    data_style_ref_cell = ws.cell(row=2, column=1) if ws.max_row >= 2 else style_ref_cell
    data_font = copy(data_style_ref_cell.font)
    data_alignment = copy(data_style_ref_cell.alignment)
    data_fill = copy(data_style_ref_cell.fill) if data_style_ref_cell.fill else None

    # 5. 更新数据（从第2行开始，第1行是标题）
    for idx, result_item in enumerate(result_list):
        ...
        for col_key, col_num in col_mapping.items():
            ...
            # 尝试转换值为Python基本类型
            try:
                # 如果是pandas的NA类型，转换为None
                if pd.isna(value):
                    cell.value = ""
                    continue

                # 如果是数字类型，确保是Python的float或int
                if isinstance(value, (int, float)):
                    cell.value = float(value)
                # 如果是numpy类型，转换为Python类型
    ...
    # 6. 应用标题行样式到Q-T列的标题
    for col_num in col_mapping.values():
        title_cell = ws.cell(row=1, column=col_num)
        title_cell.font = copy(title_font)
        title_cell.alignment = copy(title_alignment)
        if title_fill:
            title_cell.fill = copy(title_fill)
    ...
    try:
        ...
        # 尝试使用pandas作为备选方案（但会丢失样式）
        try:
            df_result = pd.DataFrame(result_list)
            df_result.to_excel(save_path, index=False, engine='openpyxl')
            print(f"⚠️ 使用pandas备选方案保存Excel结果文件至：{save_path}（样式可能丢失）")
        except Exception as e2:
            print(f"❌ pandas备选方案也失败: {e2}")
