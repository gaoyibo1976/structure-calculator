...
def _calc_ab(fcuk):
    if fcuk <= 50:
        ...
        val = round(0.8 - (fcuk - 50) * 0.002, 2)
        return val, val


# ====================== 4. 核心函数 ======================
def get_conc_params(grade):
    """
    ...
    if grade in CONC_BASE:
        ...
        params["α1"], params["β1"] = _calc_ab(grade)
        return params

    # 非标等级：全公式算
    fc, ft = _calc_fc_ft(grade)
    Ec = _calc_Ec(grade)
    α1, β1 = _calc_ab(grade)
    return {"fc": fc, "ft": ft, "Ec": Ec, "α1": α1, "β1": β1}


# ====================== 5. 快捷函数 ======================
def get_fc(grade):
    return get_conc_params(grade)["fc"]


def get_ft(grade):
    return get_conc_params(grade)["ft"]


def get_alpha1(grade):
    return get_conc_params(grade)["α1"]


def get_beta1(grade):
    return get_conc_params(grade)["β1"]


def get_Ec(grade):
    return get_conc_params(grade)["Ec"]


# ====================== 测试示例 ======================
if __name__ == "__main__":
    # 规范等级C30（查字典）
    print("C30参数：", get_conc_params(30))
    # 非标等级C37（公式算）
    print("C37参数：", get_conc_params(37))
    # 快捷函数调用
    print("C55的α1：", get_alpha1(55))